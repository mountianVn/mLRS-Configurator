...
class SerialTool(ctk.CTk):
    def __init__(self):
        ...
        # MODULE TYPE SELECT
        self.module_label = ctk.CTkLabel(self.left_panel, text="📦 Module Type", font=self.default_font)
        self.module_label.pack(pady=(5,0))

        self.module_menu = ctk.CTkComboBox(
            self.left_panel,
            values=["2.4GHz", "868/915MHz", "433MHz"],
            command=self.on_module_change,
            font=self.default_font
        )
        self.module_menu.pack(pady=(0,10))
        self.module_menu.set("868/915MHz")  # mặc định
...
#CÁI NÀY CHO PHẦN GỬI LỆNH 
    ...
    def on_module_change(self, value):

        if value == "2.4GHz":
            # RF chỉ 2.4GHz
            self.rf_menu.configure(values=["2.4GHz"])
            self.rf_menu.set("2.4GHz")

            # Mode theo ảnh
            self.mode_menu.configure(values=["50hz", "31hz", "19hz", "111hz"])
            self.mode_menu.set("31hz")

        elif value == "868/915MHz":
            self.rf_menu.configure(values=["868mhz", "915mhz"])
            self.rf_menu.set("868mhz")

            self.mode_menu.configure(values=["31hz", "19hz", "FSK"])
            self.mode_menu.set("31hz")

        elif value == "433MHz":
            self.rf_menu.configure(values=["433mhz"])
            self.rf_menu.set("433mhz")

            self.mode_menu.configure(values=["31hz", "19hz", "FSK"])
            self.mode_menu.set("31hz")

        self.write_log(f"🔄 Đã chuyển sang module: {value}")       
    ...
    # ánh xạ từ tên cấu hình trả về sang các widget và mapping
        config_map = {
           ...
           "Tx Power": (self.pow_menu, {
            ...
            "RF Band": (self.rf_menu, {"0": "2.4GHz", "1": "915mhz", "2": "868mhz", "3": "433mhz"}),
                    ...
                    if name in config_map:
                        ...
                        if value_map:
                            if index and index in value_map:
                                ...
                                # AUTO DETECT MODULE
                                rf_value = value_map[index]

                                if rf_value == "2.4GHz":
                                    self.module_menu.set("2.4GHz")
                                    self.on_module_change("2.4GHz")

                                elif rf_value in ["868mhz", "915mhz"]:
                                    self.module_menu.set("868/915MHz")
                                    self.on_module_change("868/915MHz")

                                elif rf_value == "433mhz":
                                    self.module_menu.set("433MHz")
                                    self.on_module_change("433MHz")
