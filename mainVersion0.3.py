...
class SerialTool(ctk.CTk):
    def __init__(self):
        ...
        self.cli_history_file = "cli_history.txt"
        self.cli_history = self.load_cli_history()
        self.cli_history_index = len(self.cli_history)
        ...
        self.rf_menu = ctk.CTkComboBox(self.row3, values=["433mhz","868mhz", "915mhz","2.4ghz"], width=100,font=self.default_font)
...
# Dòng thứ 10  :rx Power Sw Ch
    ...
    # cli entry  
        ...
        self.cli_entry.bind("<Up>", self.cli_history_up)
        self.cli_entry.bind("<Down>", self.cli_history_down)
        self.suggestion_box = tk.Listbox(self.bottom_row, height=5, font=("Arial", 12))
        self.suggestion_box.grid(row=1, column=0, columnspan=2, padx=5, pady=(0, 5))
        self.suggestion_box.bind("<<ListboxSelect>>", self.insert_selected_suggestion)
        self.suggestion_box.bind("<Return>", self.insert_selected_suggestion)
        self.suggestion_box.bind("<Double-Button-1>", self.insert_selected_suggestion)
        self.suggestion_box.grid_remove()  # Ẩn mặc định
        self.cli_entry.bind("<KeyRelease>", self.show_suggestions)
    # Nút gửi lệnh thủ công
        ...
        self.command_suggestions = [
            "p tx_power=", "p tx_ch_order=", "p tx_ch_source=",
            "p tx_ser_dest=", "p tx_ser_baudrate=", "p tx_power_sw_ch=",
            "p RF_BAND =", "p RF_Ortho =", "p mode =", "p bind_phrase =",
            "tx_snd_radiostat =", "p TX_MAV_COMPONENT =",
            "p rx_power=", "p RX_OUT_MODE =", "p rx_ser_baudrate =",
            "p RX_SER_LINK_MODE =", "p Rx_Snd_RadioStat =",
            "p Rx_Ser_Port =", "p RX_SND_RCCHANNEL =",
            "p RX_OUT_RSSI_CH =", "p RX_OUT_LQ_CH =",
            "p rx_power_sw_ch="
            ]
    ...
    #nút lưu
        ...
        self.save_button.grid(row=0, column=3, padx=30)#self.save_button.pack(side="right", fill="y", padx=(0,20))#pack(pady=(0, 5), padx=10, side = "right")          
    # Text log bên phải
...
#hàm cho cổng serial
    ...
    def toggle_connection(self):
        if not self.connected:
            try:
                ...
                self.beep_success()
                self.show_info("⚠️ Thông Báo","Đã Kết Nối")
    ...
    def handle_disconnection(self):
        ...
        self.load_com_ports()
        self.beep_success()        
        self.show_info("⚠️ Thông Báo","Mất Kết Nối")   
        ...
        if self.ser and self.ser.is_open:
           ...
           if val in command_map:
               ...
               #self.beep_success()
                ...
                self.show_info("Cảnh Báo", f"Không xác định {label}")
        else:
...
#phần này của nút TX_POWER
    ...
    def update_rf_band_options(self, module_type):
        if module_type == "2.4GHz":
            self.rf_menu.configure(values=["2.4GHz"], state="readonly")
            self.rf_menu.set("2.4GHz")
            self.write_log("🔒 Khóa RF Band: Module 2.4GHz chỉ hỗ trợ tần số 2.4GHz")
        elif module_type == "868MHz":
            self.rf_menu.configure(values=["868MHz"], state="readonly")
            self.rf_menu.set("868MHz")
        elif module_type == "915MHz":
            self.rf_menu.configure(values=["915MHz"], state="readonly")
            self.rf_menu.set("915MHz")
        else:
            self.rf_menu.configure(values=["868MHz", "915MHz"], state="normal")  # Mặc định
            self.rf_menu.set("868MHz")
            self.write_log("⚠️ Không xác định loại module! Sử dụng tùy chọn mặc định.") 
    ...
    def send_view_command(self):
        ...
        try:
            ...
            self.show_info("Thông Báo", "Đã cập nhật giao diện theo thiết bị!")
            self.beep_success()
        except Exception as e: 
...
# Map giá trị thực tế → index nếu thiết bị trả về không chuẩn
   ...
   #hàm này cho nút setup
    ...
    def run_next_setup_command(self):
        ...
        if self.current_setup_index < self.total_commands:
            ...
            self.after(100, self.run_next_setup_command)  # chạy lệnh tiếp theo sau 0.3 giây
        else:
  ...
  #PHẦN NÀY CHO RX
    ...
    def send_cli_command(self):
        ...
        if command:
           try:
               ...
               if not self.cli_history or self.cli_history[-1] != command:
                   self.cli_history.append(command)
                   self.append_cli_history_to_file(command)
    ...
    def cli_history_up(self, event=None):
        if self.cli_history:
            if self.cli_history_index > 0:
                self.cli_history_index -= 1
                self.cli_entry.delete(0, 'end')
                self.cli_entry.insert(0, self.cli_history[self.cli_history_index])
    def cli_history_down(self, event=None):
        if self.cli_history:
            if self.cli_history_index < len(self.cli_history) - 1:
                ...
                self.cli_history_index = len(self.cli_history)
                self.cli_entry.delete(0, 'end')
    def load_cli_history(self):
        try:
            if os.path.exists(self.cli_history_file):
                with open(self.cli_history_file, "r", encoding="utf-8") as f:
                    return [line.strip() for line in f.readlines() if line.strip()]
        except Exception as e:
            self.write_log(f"⚠️ Không thể tải lịch sử CLI: {e}")
        return []

    def append_cli_history_to_file(self, command):
        try:
            with open(self.cli_history_file, "a", encoding="utf-8") as f:
                f.write(command + "\n")
        except Exception as e:
            self.write_log(f"⚠️ Không thể ghi CLI vào file: {e}")
    ...
    def show_suggestions(self, event=None):
        ...
        matches = [cmd for cmd in self.command_suggestions if cmd.startswith(text)]
        if matches:
    ...
    def insert_selected_suggestion(self, event=None):
        try:
            selected = self.suggestion_box.get(tk.ACTIVE)
            self.cli_entry.delete(0, tk.END)
            self.cli_entry.insert(0, selected)
            self.suggestion_box.grid_remove()
        except:
            pass
    ...
    #nhận diện module
    def detect_module_type(self):
        if not self.ser or not self.ser.is_open:
            return None
    
        try:
            self.ser.reset_input_buffer()
            self.ser.write(b"v;\n")  # Gửi lệnh lấy thông tin phiên bản
            time.sleep(0.5)
            response = ""
            while self.ser.in_waiting:
                response += self.ser.readline().decode(errors="ignore").lower()
        
            if "2.4ghz" in response:
                return "2.4GHz"
            elif "915mhz" in response:
                return "915MHz"
            elif "868mhz" in response:
                return "868MHz"
            else:
                return None
            
        except Exception as e:
            self.write_log(f"❌ Lỗi phát hiện module: {e}")
            return None
